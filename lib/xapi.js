...
class Xapi {
  ...
  async authCheck(credentials) {
    let usersTableName = process.env.USERS_TABLE || 'users';
    let uidFieldName = process.env.UID_FIELD || 'id';
    let usernameFieldName = process.env.USERNAME_FIELD || 'username';
    let passwordFieldName = process.env.PASSWORD_FIELD || 'password';
    let roleFieldName = process.env.ROLE_FIELD || 'role';
  
    let query = 'SELECT * FROM ' + usersTableName + ' WHERE id=? AND ' + passwordFieldName 
      + (yesNo.parse(process.env.MD5_PASSWORDS)? '=MD5(?) ' : '=?') + 'LIMIT 1';
    
    let params = [credentials.username, credentials.password];
  
    let result = await this.mysql.exec(query, params);
    if (result.length > 0) {
      let p = {
        sub: result[0][uidFieldName],
        uid: result[0][uidFieldName],
        username: result[0][usernameFieldName],
        role: result[0][roleFieldName]
      };
      return p;
    } else {
      return undefined;
